...
histogram(
  pontos$PFd,
  ylab = list(label="Densidade", cex=1.7),
  xlab = list(label="PS (dm)", cex=1.7),
  col = "gray",
  scales=list(cex=1.7)) +
  
  latticeExtra::layer(panel.grid(v = 0)) +
...
```{r, message=FALSE}
DECLI1 <- raster::raster("../data/Covars/SLOPE.tif")
ELEV1 <- raster::raster("../data/Covars/ELEV.tif")
VD1 <- raster::raster("../data/Covars/VD.tif")
TWI1 <- raster::raster("../data/Covars/TWI.tif")
CNBL1 <- raster::raster("../data/Covars/CNBL.tif")
```
...
for (i in seq(tmp)) {
  pontos$DECLI <- raster::extract(DECLI1, pontos, buffer = 5 * i, fun = mean)
  pontos$ELEV <- raster::extract(ELEV1, pontos, buffer = 5 * i, fun = mean)
  pontos$VD <- raster::extract(VD1, pontos, buffer = 5 * i, fun = mean)
  pontos$TWI <- raster::extract(TWI1, pontos, buffer = 5 * i, fun = mean)
  pontos$CNBL <- raster::extract(CNBL1, pontos, buffer = 5 * i, fun = mean)
  tmp[i] <- summary(lm(PFd ~ DECLI1 + ELEV1 + VD1 + TWI1 + CNBL1, pontos))$adj.r.squared
}
...
pontos$DECLI <- raster::extract(DECLI1, pontos, buffer = 50, fun = mean)
pontos$ELEV <- raster::extract(ELEV1, pontos, buffer = 50, fun = mean)
pontos$VD <- raster::extract(VD1, pontos, buffer = 50, fun = mean)
pontos$TWI <- raster::extract(TWI1, pontos, buffer = 50, fun = mean)
pontos$CNBL <- raster::extract(CNBL1, pontos, buffer = 50, fun = mean)
```
...
grid$DECLI <- raster::extract(DECLI, grid, buffer = 50, fun = mean)
grid$ELEV <- raster::extract(ELEV, grid, buffer = 50, fun = mean)
grid$VD <- raster::extract(VD, grid, buffer = 50, fun = mean)
grid$TWI <- raster::extract(TWI, grid, buffer = 50, fun = mean)
grid$CNBL <- raster::extract(CNBL, grid, buffer = 50, fun = mean)
```

```{r fig.asp=1, echo=FALSE, fig.cap = " Covariáveis topográficas utilizadas como preditoras no modelo floresta aleatória"}
par(mfrow=c(2,1))
plot(DECLI1,main='Declividade')
plot(DECLI, main='Declividade reamostrado 100 m')
```
...
vario_fit <- georob::georob(
   PFd ~ DECLI + ELEV + VD + TWI + CNBL, pontos, locations = ~ X + Y, variogram.model = 'RMexp', 
 param = c(variance = vario_fit$variogram.object[[1]]$param[['variance']], 
...
par(font.lab=6) 
plot(vario, type = "b", annotate.npairs = TRUE, font.axis = 6, ylab = "Semivariância", xlab = "Distância (lag)", cex.lab = 1.7, cex.axis = 1.7)
...
prof_vario_fit_nug_scale <- 
  georob::profilelogLik(object = vario_fit_nug, values = data.frame(scale = seq(10, 300, by = 10)))

par(font.lab=6)
plot(loglik ~ scale, prof_vario_fit_nug_scale, type = "l", xlab = 'Alcance', cex.lab=1.7, cex.axis=1.7,  font.axis = 6)
abline(v = summary(vario_fit_nug)$param.aniso[[1]]['scale', ], lty = c("dashed", rep('dotted', 2)), col = 'red')
abline(h = vario_fit_nug$loglik - 0.5 * qchisq(0.95, 1), lty = "dotted")
```

```{r}
prof_vario_fit_scale <- 
  georob::profilelogLik(object = vario_fit, values = data.frame(scale = seq(10, 300, by = 10)))

par(font.lab=6)
plot(loglik ~ scale, prof_vario_fit_scale, type = "l", xlab = 'Alcance', cex.lab=1.7, cex.axis=1.7,  font.axis = 6)
abline(v = summary(vario_fit)$param.aniso[[1]]['scale', ], lty = c("dashed", rep('dotted', 2)), col = 'red')
