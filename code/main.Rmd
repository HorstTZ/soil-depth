...
```{r}
newdata <- sp::spsample(ProdutividadePistola, 10000, type = 'regular')
```
...
grid<- 
  sp::SpatialPointsDataFrame(
    coords = grid@coords, 
    data = data.frame(grid$PFprediction),
...
O variograma amostral foram computado através da função `georob::sample.variogram(method"metheron"`. O estimador para semivariãncia foi Metheron (método dos momentos).
Para a obtenção dos parâmetros foi utilizado um corte de 50% da distância máxima entre os pontos, armazenada no objeto `distmax`, excluindo os pares de longo alcance.
