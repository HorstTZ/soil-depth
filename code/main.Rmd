...
```{r}
rm(list = ls())
pontos <- read.csv('../data/Gateados-campo.csv', dec = ".", sep= ";", stringsAsFactors = FALSE)
...
```{r, message=FALSE}
DECLI <- raster::raster("../data/Covars/DECLI1.tif")
ELEV <- raster::raster("../data/Covars/ELEV1.tif")
VD <- raster::raster("../data/Covars/VD1.tif")
TWI <- raster::raster("../data/Covars/TWI1.tif")
CNBL <- raster::raster("../data/Covars/CNBL1.tif")
```
...
pontos$DECLI <- raster::extract(DECLI, pontos, buffer = 10, fun = mean)
pontos$ELEV <- raster::extract(ELEV, pontos, buffer = 10, fun = mean)
pontos$VD <- raster::extract(VD, pontos, buffer = 10, fun = mean)
pontos$TWI <- raster::extract(TWI, pontos, buffer = 10, fun = mean)
pontos$CNBL <- raster::extract(CNBL, pontos, buffer = 10, fun = mean)
```
