...
pontos$ASP <- extract(ASP,  pontos)
pontos$CNBL <- extract(CNBL, pontos)
pontos$DECLI <- extract(DECLI, pontos)
pontos$ELEV <- extract(ELEV, pontos)
pontos$LS <- extract(LS, pontos)
pontos$PLC <- extract(PLC, pontos)
pontos$RSP <- extract(RSP, pontos)
pontos$TPI <- extract(TPI, pontos)
pontos$TRI <- extract(TRI, pontos)
pontos$TWI <- extract(TWI, pontos)
pontos$VD <- extract(VD, pontos)
pontos$VDCN <- extract(VDCN, pontos)
pontos$RFPF <- extract(RFPF, pontos)

str(pontos)
```

Dos 108 ha, 73 são de área plantada. Para fins de ordenamento de produção, a área em questão possui 12 parcelas fixas de inventário contínuo (PIC) de 500 metros quadrados, classificados conforme a produtividade em níveis 1, 2, 3 e 4, sendo 1 o sítio com melhor produtividade e 4 de pior produtividade (Figura 2).
...
Os níveis de produtividade foram atribuidos aos polígonos e, nos polígonos que não possuem PIC, o valor de sítio foi estimado com base na altura das árvores amostradas á campo e das PCIs vizinhas.
...
sp::spplot(ProdutividadePistola, scales = list(draw = TRUE), main = "Mapa de índices de Sítio")
