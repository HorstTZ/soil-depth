...
<!-- ASR: Suponho que seria importante explicar a razão dessa decisão. -->
...
<!-- ASR: Me parece que a eliminação dos pontos amostrados fora das áreas de produção deveria ser feita aqui, logo no início do processamento. -->
...
<!-- ASR: Quando a distribuição é bimodal faz mais sentido falar sobre as duas modas. A média é pouco informativa. Seria importante explicar a concentração de valores em 10 dm -- seria efeito da amostragem (dados censurados)? -->
...
<!-- ASR: Parece-me importante relatar a razão da agregação. -->
...
<!-- ASR: TIF é um formato de arquivo de armazenamento de dados no computador. Ele não é usado no R. -->
...
<!-- ASR: Seria interessante explicar como são definidos os talhões, sobretudo porque eles são explorados como covariável. -->
...
<!-- ASR: Na verdade usou a função raster::raster() e o nome do objeto é 'Sitio'. -->
...
```{r}
# ASR: Figura com pontos. Veja que os mesmos 102 pontos continuam no objeto. Isso significa que os
#     pontos com NAs não foram removidos. Veja abaixo.
plot(Sitio, main = paste("n = ", length(pontos)))
points(pontos@coords)
```

```{r}
is.na(pontos$Sitio) %>% any
```

```{r}
# ASR: Remoção dos pontos com NAs
pontos <- pontos[!is.na(pontos$Sitio), ]
```

<!-- ASR: Importante gerar o histograma e estatísticas descritivas dos dados após remoção dos NAs. -->
```{r, fig.asp=1}
...
<!-- ASR: Acho que não é nset = 101 amostras pois existem pontos com NA que foram removidos. Por isso sempre é bom definir essas variáveis programaticamente. Veja abaixo. -->
