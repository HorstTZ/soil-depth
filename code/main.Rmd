...
histogram(
  pontos$PFd,
  ylab = list(label="Densidade", cex=1.7, family="A"),
  xlab = list(label="PS (dm)", cex=1.7, family="A"),
  col = "gray",
  scales=list(cex=1.7)) +
...
distmax <-dist(pontos@coords) %>% max() / 3
limites <- seq(0, distmax, length.out = 20)
vario <- georob::sample.variogram(PFd ~ DECLI + ELEV + VD + TWI + CNBL,
...
```{r}
prof_reml_fit_scale <- 
  georob::profilelogLik(object = vario_fit_error, values = data.frame(scale = seq(10, 250, by = 10)))

plot(loglik ~ scale, prof_reml_fit_scale, type = "l", xlab = 'Alcance', cex.lab=1.5, cex.axis=1.5)
