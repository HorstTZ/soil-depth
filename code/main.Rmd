...
library(lattice)
H1 <- histogram(pontos$PFd) + latticeExtra::layer(panel.grid(v = 0)) + latticeExtra::layer(panel.rug(pontos$PFd))

H2 <- histogram(pontos$DECLI) + latticeExtra::layer(panel.grid(v = 0)) + latticeExtra::layer(panel.rug(pontos$DECLI))

H3 <- histogram(pontos$ELEV) + latticeExtra::layer(panel.grid(v = 0)) + latticeExtra::layer(panel.rug(pontos$ELEV))

H4 <- histogram(pontos$TWI) + latticeExtra::layer(panel.grid(v = 0)) + latticeExtra::layer(panel.rug(pontos$TWI))

H5 <- histogram(pontos$CNBL) + latticeExtra::layer(panel.grid(v = 0)) + latticeExtra::layer(panel.rug(pontos$CNBL))

H6 <- histogram(pontos$VD) + latticeExtra::layer(panel.grid(v = 0)) + latticeExtra::layer(panel.rug(pontos$VD))

plot(H1, split = c(1, 1, 2, 3))
plot(H2, split = c(2, 2, 2, 3), newpage = FALSE)
plot(H3, split = c(1, 2, 2, 3), newpage = FALSE) 
plot(H4, split = c(2, 1, 2, 3), newpage = FALSE) 
plot(H5, split = c(1, 3, 2, 3), newpage = FALSE) 
plot(H6, split = c(2, 3, 2, 3), newpage = FALSE) 
...
```{r, message=FALSE}
DECLI <- raster::raster("../data/Covars/SLOP.tif")
...
for (i in seq(tmp)) {
  pontos$DECLI <- raster::extract(DECLI, pontos, buffer = 5 * i, fun = mean)
  ...
  tmp[i] <- summary(lm(PFd ~ DECLI + ELEV + VD + TWI + CNBL, pontos))$adj.r.squared
}
...
pontos$DECLI <- raster::extract(DECLI, pontos, buffer = 50, fun = mean)
...
matriz <-pontos@data[,6:11]
matriz.cor=cor(matriz)
matriz.cor

lm(PFd ~ DECLI, data = pontos@data) %>% summary()
...
```{r}
lm(PFd ~ CNBL, data = pontos@data) %>% summary()
```
...
pred_ponto <- predict(
  vario_fit_error, newdata = grid, type = "response", signif = 0.95, na.rm = TRUE,
...
validacao <- georob::cv(vario_fit_error, nset = nset, replace = TRUE)
summary(validacao)
```
