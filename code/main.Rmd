...
```{r sample grid}
grid <- sp::spsample(pedologia25, 10000, type = 'regular') #CRIAR O GRID: Suporte de predição pontual
grid <- 
  sp::SpatialPointsDataFrame(
    coords = grid@coords, 
    data = data.frame(
      um = sp::over(grid, pedologia25) %>% unlist(), #amostra dos dados pedologicos
      geo = sp::over(grid, geologia50) %>% unlist()), #amostra dos dados geologicos
    proj4string = grid@proj4string) #definir projeção
str(grid)
```

```{r colnames}
colnames(grid@coords) <- colnames(pontos400in@coords) #deixar com nomes diferentes
