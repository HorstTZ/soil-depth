...
<!-- A figura deve aparecer após a primeira referência -->
...
<!-- Acho melhor manter somente a profundidade do solum, explicando a questão do suporte amostral. -->
...
<!-- Não há necessidade de assumir que os dados venham de uma distribuição normal. Ademais, a seção sobre 
amostragem não parece o local mais adequado para falar sobre isso. Favor rever. -->
...
<!-- Ao invés de efeitos aleatórios é melhor dizer 'efeitos estocásticos' para evitar confusão com o termo do erro -->
...
<!-- Isso significa que os efeitos fixos foram tomados como sendo variáveis (não-constantes) na área. -->
...
<!-- Os dados estão armazenados no formato tif. RasterLayer é apenas um formato de objeto do R. -->
...
```{r lm-residuos, fig.cap=" Ajuste de dados preditos e observados", fig.asp=1}
par <- par(mfrow = c(2,2))
