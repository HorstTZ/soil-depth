...
Com uma análise preliminar avaliei a distribuição de freqüênciasnos dados de profundidade do solum. 
Apesar das amostras apresentam uma distribuição bimodal (FIgura 2), visando ao atendimento de restrições teóricas ao emprego da geoestatística, assumi que os dados provém de uma distribuição normal.

```{r, echo = FALSE, asp = 1, fig.cap="2 Histograma de frequências da profundidade do solum"}
...
Para isso foi utilizado o modelo linear misto de variação espacial $y(\boldsymbol{s}_i)$, denotado por
...
Para utilizar esse modelo, é necessário supor que os dados são uma realização de um campo aleatório $Y(\boldsymbol{s}_i)$ com distribuição normal que podem ser descritos como a combinação aditiva de efeitos fixos, efeitos aleatórios e erro aleatório independente.
...
$\varepsilon(\boldsymbol{s}_i)$ é um erro (ou ruído) independente e identicamente distribuído (iid), descrito por uma distribuição Gaussina de probabilidade, cujo parâmetro desconhecido de escala é $\tau$. Tradicionalmente, $\tau^2$ é chamado de efeito pepita ou ainda de variância residual.
...
Cada PIC é classificada em função da média de altura das 100 árvores de maior perÍmetro basal da parcela, expressa como altura dominante (Hdom). Em função da Hdom e seus incrimentos anuais é estabelecido o índices de sítio (Sitio). Esse índice é então atribuido à todo o talhão.
...
```{r fig.asp=1, fig.width=7, fig.height=7, fig.cap="6 Gráfico da função de semivariância"}
...
```{r, asp = 1, fig.cap="7 variograma ajustado"}
plot(vario, xlab = 'Distância de separação (m)', ylab = 'Semivariância', annotate.npairs = TRUE)
lines(vario_fit, col = "red", lty = 'dashed')
```

Considerando que durante a obteção dos dados de campo as informações de profundidade do solum os valores foram obtidos em decímetros, considerei que a variância do erro de medida é igual a 0.5 dm devido ao arredondamento dos valores. Assim, o erro de medida corresponde a aproximadamente 62% do parâmetro `nugget`. A variância restante foi atribuída a variação espacial não auto-correlacionada espacialmente - não capturada pelo plano amostral `snugget`.
