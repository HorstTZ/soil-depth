...
```{r, message=FALSE}
DECLI <- raster::raster("../data/Covars/1m/DECLI.tif")
ELEV <- raster::raster("../data/Covars/1m/ELEV.tif")
VD <- raster::raster("../data/Covars/1m/VD.tif")
TWI <- raster::raster("../data/Covars/1m/TWI.tif")
CNBL <- raster::raster("../data/Covars/1m/CNBL.tif")
```
...
for (i in seq(tmp)) {
  ...
  tmp[i] <- summary(lm(PFd ~ DECLI + ELEV + VD + TWI, pontos))$adj.r.squared
}
...
pontos$DECLI <- raster::extract(DECLI, pontos, buffer = 5, fun = mean)
pontos$ELEV <- raster::extract(ELEV, pontos, buffer = 5, fun = mean)
pontos$VD <- raster::extract(VD, pontos, buffer = 5, fun = mean)
pontos$TWI <- raster::extract(TWI, pontos, buffer = 5, fun = mean)
pontos$CNBL <- raster::extract(CNBL, pontos, buffer = 5, fun = mean)
```
